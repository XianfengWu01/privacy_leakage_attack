...
def _build_dataset_vision(cfg_data, split, can_download=True):
    ...
    elif cfg_data.name == "Retina":
        dataset = get_dataset(root=cfg_data.path, split=split)
        dataset.lookup = dict(zip(list(range(len(dataset))), dataset.labels))
    else:
...
def load_hf_data(cfg, phase):
    ...
    if dataset_name == 'Retina':
        ...
        hf = h5py.File(os.path.join(data_path, dataset_name + '_' + phase + '.h5'), 'r')
        d = hf['examples']
        ...
        hf = h5py.File(os.path.join(data_path, 'COVID_data', dataset_name + '_' + phase + '.h5'), 'r')
        pixels = hf['pixels']
        label = hf['label']
    ...
    def __init__(self, data, label, data_aug=False, phase = 'train'):
        ...
        if phase == 'train':
            transform = torch.utils.data.transforms.Compose([
                torch.utils.data.transforms.RandomResizedCrop((224, 224), scale=(0.05, 1.05)),
                torch.utils.data.transforms.RandomHorizontalFlip(),
                torch.utils.data.transforms.ToTensor(),
                torch.utils.data.transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
            ])
            
        else:
            transform = torch.utils.data.transforms.Compose([
                torch.utils.data.transforms.Resize((224, 224)),
                torch.utils.data.transforms.ToTensor(),
                torch.utils.data.transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
            ])
...
def get_dataset(cfg, split):
  ...
  if split == 'test':
    ...
    # tests = torch.utils.data.DataLoader(dataset=test_set, batch_size=20, shuffle=False, drop_last=True)  #Miao used batch size 20

    val_set = MyDataset(val_pixels, val_label, data_aug=data_aug_phase, phase='val')
    # valids = torch.utils.data.DataLoader(dataset=val_set, batch_size=40, shuffle=False, drop_last=True)
    ...
    fixed_val_set = MyDataset(fixed_data, fixed_label, data_aug=False, phase = 'val')
    fixed_valids = torch.utils.data.DataLoader(dataset=fixed_val_set, batch_size=10, shuffle=True, drop_last=False)

    return test_set
  else :
    trainset = MyDataset(train_pixels, train_label, data_aug=data_aug_phase, phase='test')
    return trainset
