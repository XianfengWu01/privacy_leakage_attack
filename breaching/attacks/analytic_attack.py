...
class AprilAttacker(AnalyticAttacker):
    ...
    def closed_form_april(model, shared_data, solver="gelsd"):
        ...
        qkv_w = model.blocks[0].attn.qkv.weight.detach().double()              # minimal test
        ...
        pos_embed = model.pos_embed.detach().squeeze().double()              # minimal test
        ...
        em_w = model.patch_embed.proj.weight.detach().double()              # minimal test
        em_w = em_w.reshape((em_w.shape[0], -1))
        # em_b = model.model.patch_embed.proj.bias.detach().double().cpu()
        em_b = model.patch_embed.proj.bias.detach().double()                # minimal test
